...
# The script works by looking at the current file directory in the `main` branch and      #
...
# Function to filter root items to get rid of those including a "." (i.e. ".snippets"), "js", "images"
...
# Function to get the previous file paths from `main`
def fetch_previous_file_paths():
  previous_file_paths = []
  # Get the sha of the `main` branch so it can be used in the following calls
  sha = requests.get("https://api.github.com/repos/moondance-labs/tanssi-docs/branches/main").json()["commit"]["sha"]

  # Get file structure from the `main` branch
  rootTree = requests.get("https://api.github.com/repos/moondance-labs/tanssi-docs/git/trees/" + sha + "?recursive=1").json()["tree"]

  for item in rootTree:
    omit_dirs = [".snippets", "js", "images", ".", "README.md"]
    path = item["path"]
...
for previous_file in previous[:]:
    for current_file in current[:]:
        ...
        if (current_file_name in previous_file):
          ...
          if (previous_file in previous):
            previous.remove(previous_file)
        
          if (current_file in current):
            current.remove(current_file)
...
prev_mapping = mkdocs_yaml["plugins"][4]["redirects"]["redirect_maps"]
...
mkdocs_yaml["plugins"][4]["redirects"]["redirect_maps"] = new_dict
